...
- (void)updateProgress {
    [(XMBRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
        self.indicator = [[[XMBRoundProgressView alloc] initWithDefaultSize] autorelease];
    }
...
+ (XMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	XMBProgressHUD *hud = [[XMBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[XMBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		XMBProgressHUD *HUD = (XMBProgressHUD *)viewToRemove;
