...
/** Get the condition used for matching
 ...
 @see StringMatch
 */
@property (nonatomic, strong) id <IMatcher> condition;
...
/** Get the current logic operator, could be null if the expression has no other concatenated expressions
 @see LogicOperator
 */
@property (nonatomic, strong) LogicOperator *logicOp;
@property (nonatomic, strong) MatchExpression *parent;

/** Get the next expression chained to the current one.
 */
@property (nonatomic, strong) MatchExpression *next;
