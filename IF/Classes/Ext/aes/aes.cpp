...
static const std::string key ="U2FsVkX1+e21vUR=";
...
static void dor(byte *a, const byte *b, int n)
{
    int i;
    for (i=0; i<n; i++) {
        a[i] ^= b[i];
...
static void expand_key(const byte *key, byte *keys) {
    ...
    while (bytes<176) {       // Until we have 176 bytes of expanded key, we do the following:
        ...
        dor(t,keys+bytes-16,4);            // We exclusive-or t with the four-byte block 16 bytes before the new expanded key.
        ...
        for (j=0; j<3; j++) {
            ...
            dor(t,keys+bytes-16,4);            // We exclusive-or t with the four-byte block n bytes before
            memcpy(keys+bytes,t,4);            // This becomes the next 4 bytes in the expanded key
            bytes+=4;                          // Keep track of how many expanded key bytes we've added
        }
    }
...
std::string getAesKey(double t)
{
    int t1 =fmod(t,10)+ 2;
    double t2 = t / 10;
    
    std::string s1 = key.substr(0,t1);
    std::string s2 = key.substr(t1);
    std::string askey="";
    if (fmod(t2,2) < 1) {
        askey.assign(s1.rbegin(), s1.rend());
        return askey+s2;
    } else {
        askey.assign(s2.rbegin(), s2.rend());
        return s1+askey;
    }
    return key;
}
...
// Test AES
std::string AES::encrypt(const std::string& word,double time) // 加密函数
{
    std::string key2 =getAesKey(time);
    byte* key =(byte*) malloc(16);
    memset(key, 0, 16);
    memcpy(key, key2.c_str(), key2.length());
    int nLen =word.length();
    int size = word.length()+(16-nLen%16);
    int block =size/16;
    byte* byteword =(byte*) malloc(size);
    memset(byteword, 0, 16);
    memcpy(byteword, word.c_str(), word.length());
    for (int n=0;n<(16-nLen%16);n++)
    {
        byteword[nLen+n] = (16-nLen%16);
    }
    int i=0;
    int j=0;
    byte outEncrypt[size];
    byte encrypted[16], decrypted[16],temp[16];
    for (int i=0;i<block;i++)
    {
        for (j=0;j<16;j++)
            temp[j] = byteword[i*16+j];
        EncryptAES(temp,key,encrypted);
        memcpy(outEncrypt+i*16,encrypted,16);
    }
    char * out = NULL;
    cocos2d::base64Encode(outEncrypt, size, &out);
...
std::string AES::decrypt(const std::string& word,int lenth,double time) // 加密函数
{
    unsigned char * msg = NULL;
    cocos2d::base64Decode((unsigned char*)word.c_str(), word.length(), &msg);
    std::string key2 =getAesKey(time);
    byte* key =(byte*) malloc(16);
    memset(key, 0, 16);
    memcpy(key, key2.c_str(), key2.length());
    ...
    int size = lenth;
    int block =size/16;
    byte* outDecrypt=(byte*) malloc(size);
    memset(outDecrypt, 0, size);
    byte encrypted[16],temp[16];
    int j,n;
    for (int i=0;i<block;i++)
    {
        for (j=0;j<16;j++)
            temp[j] = msg[i*16+j];
        DecryptAES(temp,key,outDecrypt+i*16);
    }
    for (n=size-1;n>=size-16;n--)
    {
        if (1<=(int)outDecrypt[n]&&16>=(int)outDecrypt[n])
        {
            outDecrypt[n] = 0;
        }
        else
    ...
    std::string strRes = (char*)outDecrypt;
    free(outDecrypt);
    free(key);
    free(msg);
