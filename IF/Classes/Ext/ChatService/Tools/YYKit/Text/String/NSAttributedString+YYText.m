...
+ (NSMutableAttributedString *)attachmentStringWithEmojiImage:(UIImage *)image fontSize:(CGFloat)fontSize  needOffset:(CGFloat)y  andImageFrame:(CGRect)vImageRect {
    ...
    CGFloat ascent = 0;//YYEmojiGetAscentWithFontSize(fontSize);
    CGFloat descent = 0;//YYEmojiGetDescentWithFontSize(fontSize);
//    CGRect bounding = YYEmojiGetGlyphBoundingRectWithFontSize(fontSize);
//    bounding.size .width = vImageRect.size.width;
    CGRect bounding =vImageRect;
 
    YYTextRunDelegate *delegate = [YYTextRunDelegate new];
    delegate.ascent = ascent;
    delegate.descent = descent+y;
    ...
    if (hasAnim) {
        ...
        view.autoPlayAnimatedImage = YES;
        view.frame = CGRectMake(bounding.origin.x, bounding.origin.y, bounding.size.width, bounding.size.height);
        view.image = image;
...

{
    ...
    if (H==0) {
        ...
        descent=descent-2;
        delegate.width = bounding.size.width + 2 * bounding.origin.x+H;
    }
