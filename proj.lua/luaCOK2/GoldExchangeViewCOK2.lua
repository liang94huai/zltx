...
require "commonCOK2"
require "GoldExchangeCellCOK2"
require "GoldExchangeCell1COK2"
...
function GoldExchangeView:init(parent,path,params)
    ...
    if nil ~= GoldExchangeLuaView["m_listNode"] then
    	...
    	if nil ~= self.m_listNode then
            self.m_listNode:setContentSize(CCSize(self.m_listNode:getContentSize().width,self.m_listNode:getContentSize().height + 60));
    ...
    self:initParticale()
    print("self:initViewSkeleton()-469")
    ...
    if self.data[19] == "jp_fight" then
        if nil~= self.m_ani1Layer then
            ...
            if nil ~= particles1 then
                ...
                particles1:setPosition(CCPoint(winSize.width*0.5,winSize.height));
            end
            ...
            if nil ~= self.m_ani2Layer then
                ...
                for i=1,4 do
                    ...
                    if nil ~= particle1 then
                        ...
                        particle1:setPosition(CCPoint(size.width*0.5,size.height*0.5))
                    end
                    ...
                    if nil ~= particle2 then
                        ...
                        particle2:setPosition(CCPoint(size.width*0.5,size.height*0.5))
                    end
                end
            end
        end
    elseif self.data[19] == "oneyear" then
        if nil~= self.m_anlayer10 then
            local function tick1()
                if nil ~= self.m_anlayer10 then
                    self.m_anlayer10:removeAllChildren(true)
        ...
        if nil~= self.m_anlayer11 then
            local function tick2()
                if nil ~= self.m_anlayer11 then
                    self.m_anlayer11:removeAllChildren(true)
        ...
        if nil~= self.m_anlayer12 then
            local function tick3()
                if nil ~= self.m_anlayer12 then
                    self.m_anlayer12:removeAllChildren(true)
        ...
        if nil~= self.m_anlayer13 then
            local function tick4()
                if nil ~= self.m_anlayer13 then
                    self.m_anlayer13:removeAllChildren(true)
        ...
        if nil~= self.m_anlayer14 then
            local function tick5()
                if nil ~= self.m_anlayer14 then
                    self.m_anlayer14:removeAllChildren(true)
